...
public class RedisServer {
    ...
    private void acceptClient(ServerSocketChannel server,Selector selector) throws Exception {
    ...
    private void readClient(SelectionKey key) {
        try {
            ...
            if(bytes > 0) {
            while(true) {
        List<String> command =session.getParser().readCommand(buffer);
        if(command == null) {
            break;
        }
        System.out.println("Received: " + command);
        String response =processor.process(command,database,session);
        ByteBuffer responseBuffer =ByteBuffer.wrap(response.getBytes());
        session.getWriteQueue().add(responseBuffer);
